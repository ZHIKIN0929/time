...
function App() {
  ...
  const setData = () => {
    setParagraph(quotes[Math.floor(Math.random() * quotes.length)]);
  };
  ...
  return (
    <div className="w-full h-screen flex justify-center items-center flex-col bg-[#5CA4A9]">
      <div className="text-white text-3xl pl-2 select-none">
        {moment().format("LT")}
      </div>
      <div className="mt-3"></div>
      <div className="text-4xl text-center mx-[6rem] leading-[3rem] text-white select-none">
        {paragraph}
      </div>
      <div className="flex">
        <Icon
          ...
          onClick={setData}
          className="mt-5 w-16 h-16 text-white"
        />

        <CopyToClipboard text={paragraph}>
          <Icon
            icon="akar-icons:copy"
            className="w-16 h-16 hover:cursor-pointer text-white mt-4 pl-3"
          ></Icon>
        </CopyToClipboard>
      </div>
    </div>
